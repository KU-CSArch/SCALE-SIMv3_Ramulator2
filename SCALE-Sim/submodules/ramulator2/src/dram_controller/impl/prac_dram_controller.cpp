        ...
        if (request_found) {
            ...
            if (req_it->command == req_it->final_command) {
                if (req_it->type_id == Request::Type::Read) {
                    ...
                    // Compute a write "depart" time using DRAM timing values (CWL + BL)
                    // There is no m_write_latency member in IDRAM; approximate with nCWL + nBL
                    Clk_t write_latency = 0;
                    try {
                        write_latency = m_dram->m_timing_vals("nCWL") + m_dram->m_timing_vals("nBL");
                    } catch (...) {
                        // Fallback to read latency if timing names are unavailable
                        write_latency = m_dram->m_read_latency;
                    }
                    req_it->depart = m_clk + write_latency;
                    // Log and push to pending so callbacks are invoked when write completes
                    printf("WR: 0x%lx %ld %ld\n", req_it->addr, req_it->arrive, req_it->depart);
                    pending.push_back(*req_it);
                }
        ...
        if (pending.size()) {
            ...
            if (req.depart <= m_clk) {
                ...
                if (req.depart - req.arrive > 1) {
                    printf("RD: 0x%lx %ld %ld\n",req.addr,req.arrive,req.depart);
